...
ob_start();
	...
	if(($_SESSION['sudahloginadmin']==true)&&($_SESSION['role']=='ADMIN')){
		...
		$idp = $_GET['id'];
		$get_pegawai = mysql_query("SELECT * FROM pegawai WHERE id_pegawai='$idp'");
...
<head>
	...
	<title>DIGITAL MENGAWASI BC GRESIK | EDIT DATA PEGAWAI</title>
	<meta name="description" content="DIGITAL MENGAWASI BC GRESIK EDIT DATA PEGAWAI" />
    ...
    <div class="wrapper theme-1-active pimary-color-blue">
		...
		<div class="fixed-sidebar-left">
			<ul class="nav navbar-nav side-nav nicescroll-bar">
				...
				<li>
					<a href="menupegawai.php" class="active"><div class="pull-left"><i class="zmdi zmdi-accounts mr-20"></i><span class="right-nav-text">Kelola Pegawai</span></div><div class="clearfix"></div></a>
				</li>
				<li>
					...
					<div class="col-sm-6">
							<div class="panel panel-default card-view">
								<div class="panel-heading">
									<div class="pull-left">
										<h6 class="panel-title txt-dark">Edit Data Pegawai</h6>
									</div>
										...
										<div class="form-wrap">
											<form class="form-horizontal" method="post" action="" name="create">
												<div class="form-group">
													<label class="control-label mb-10 col-sm-3" for="id">NIP:</label>
												<div class="col-sm-6">
													<input type="text" class="form-control" name="id" style="text-transform: uppercase" maxlength="18" value="<?php echo $data_pegawai['id_pegawai'];?>">
												</div>
												</div>
												<div class="form-group">
													<label class="control-label mb-10 col-sm-3" for="nama">Nama Pegawai:</label>
												<div class="col-sm-6">
													<input type="text" class="form-control" name="nama" value="<?php echo $data_pegawai['nama_pegawai'];?>">
												</div>
												</div>
												<div class="form-group">
													<label class="control-label mb-10 col-sm-3" for="role">Role:</label>
												<div class="col-sm-6">
													<select class="form-control" name="role">
														<?php if($data_pegawai['role_pegawai'] == "ADMIN"){?>
												...
												<div class="form-group mb-0"> 
													<div class="col-sm-offset-3 col-sm-10">
													  <button name="submit" type="submit" class="btn btn-primary btn-anim"><i class="icon-rocket"></i><span class="btn-text">Submit</span></button>
													</div>
												</div>
												</form>
													<?php
														if (isset($_POST['submit'])==true){
															...
															$idlama = $data_pegawai['id_pegawai'];
															$role = $_POST['role'];
															$nama = strtoupper($_POST['nama']);
															
															if($id != "" && $nama != ""){
																$update = mysql_query("UPDATE pegawai SET id_pegawai='$id', role_pegawai='$role', nama_pegawai='$nama' WHERE id_pegawai='$idlama'");
																if($update){
																	echo '<script language="javascript">alert("Data pegawai berhasil diubah!");document.location="menupegawai.php"</script>';
																}
																else{
																	echo '<script language="javascript">alert("Data pegawai gagal diubah!");</script>';
																}
															}
															else{
